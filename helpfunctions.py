...
class ReplayBuffer(object):
    ...
    def _encode_sample_ensemble(self, idxes):
...
class PrioritizedReplayBufferEnsemble(ReplayBuffer):
    def __init__(self, size, alpha, numHeads=1, bProb=1.0):
        ...
        numHeads : int
            How many network heads for bootstrapping
        bProb : float
            Probability of learning from experience for one head
        See Also
        --------
        ReplayBuffer.__init__
        """
        super(PrioritizedReplayBufferEnsemble, self).__init__(size)
        assert alpha >= 0
        self._alpha = alpha
        ...
        self.bernoulli_probability = bProb
    ...
    def sample(self, batch_size, beta):
        ...
        encoded_sample = self._encode_sample_ensemble(idxes)
